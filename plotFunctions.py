...
def orsum_readResultFile(inputFile):
    ...
    palette = sns.cubehelix_palette(
        light=0.8, n_colors=rankMax, as_cmap=False, reverse=True
    )
    palette_cmap = sns.cubehelix_palette(
        light=0.8, n_colors=rankMax, as_cmap=True, reverse=True
    )
    ranksColor = []
    for rank in ranks:
    ...
    # CREATE DATA FRAME
    df = ps.DataFrame(
        {"sizes": sizes, "labels": labels, "ranks": ranks, "colors": ranksColor}
    )
...
def orsum_heatmap(
    allRanks_array, df, nbTerm, plotName, conditionName, palette_cmap, ticks
):
    """
...
def orsum_heatmap_quartile_quantitative(
    quartiles_array, df, nbTerm, plotName, conditionName
):
    """
    Create and save heatmap of the results of each analysis from orsum.py.
    ...
    # Color for heatmap
    nbMax = 4
    quartilePalette_cmap = sns.cubehelix_palette(
        light=0.8, n_colors=nbMax, as_cmap=True, reverse=True
    )
    bounds = [1, 2, 3, 4, 5]
...
def orsum_plot(inputFile, outputDir, threshold):
    ...
    df, allRanks_array, resultsId, palette_cmap = orsum_readResultFile(
        inputFile=inputFile
    )
    ...
    orsum_barplot(
        df=df,
        nbTerm=threshold,
        sizeMax=df["ranks"].max(),
        sizeMin=df["ranks"].min(),
        plotName=barplotName,
        ticks=boundariesCB,
    )
    ...
    orsum_heatmap_quartile_quantitative(
        quartiles_array=allQuartiles_array,
        df=df,
        nbTerm=threshold,
        plotName=heatmapQuartQuantitativeName,
        conditionName=resultsId,
    )
